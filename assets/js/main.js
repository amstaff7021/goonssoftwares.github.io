...
(function () {
  ...
  // Align product videos horizontally by equalizing bullets height
  function alignProductVideos() {
    const lists = Array.from(document.querySelectorAll('#products .product-card .bullets'));
    if (!lists.length) return;
    // reset first to measure natural heights
    lists.forEach((el) => (el.style.minHeight = ''));
    const maxH = lists.reduce((m, el) => Math.max(m, el.offsetHeight), 0);
    lists.forEach((el) => (el.style.minHeight = maxH + 'px'));
  }
  // Run after paint
  requestAnimationFrame(alignProductVideos);
  let raf;
  window.addEventListener('resize', () => {
    cancelAnimationFrame(raf);
    raf = requestAnimationFrame(alignProductVideos);
  });

  // Analytics: track Discord joins
  function trackDiscordJoin(pos) {
    try {
      if (typeof window.plausible === 'function') {
        window.plausible('join_discord_click', { props: { position: pos || 'unknown' } });
      }
    } catch (_) {}
  }
  document.querySelectorAll('a[data-analytics="join-discord"]').forEach((a) => {
    a.addEventListener('click', () => trackDiscordJoin(a.dataset.pos || 'unknown'));
  });
})();
